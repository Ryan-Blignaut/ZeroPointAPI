...
public class Mesh
{
	...
	public void end()
	{
		...
		if (indicesCount > 0)
		{
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo);
			BufferRendererAccessor.setCurrentElementBuffer(ibo);
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.limit(indicesCount * 4), GL_DYNAMIC_DRAW);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
			BufferRendererAccessor.setCurrentElementBuffer(0);
		}
...
public class Mesh
{
	...
	private ByteBuffer vertices;
	private long verticesPointer, verticesPointerStart;
	private int vertexComponentCount;
...
//			int attribute = attributes[i].size;
			glEnableVertexAttribArray(i);
			glVertexAttribPointer(i, attrib.getSize(), GL_FLOAT, false, stride, offset);
...
//			memCopy(memAddress(vertices), memAddress(newVertices), vertexComponentCount * 4L);
		...
		if (indicesCount * 4 >= indices.capacity())
		{
			...
			ByteBuffer newIndices = BufferUtils.createByteBuffer(newSize);
			memCopy(memAddress(indices), memAddress(newIndices), indicesCount * 4L);
	...
	public void end()
	{
		...
		{
			glBindBuffer(GL_ARRAY_BUFFER, vbo);
			glBufferData(GL_ARRAY_BUFFER, vertices.limit(getVerticesOffset()), GL_DYNAMIC_DRAW);
			glBindBuffer(GL_ARRAY_BUFFER, 0);
