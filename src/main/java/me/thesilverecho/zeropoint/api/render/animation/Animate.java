package me.thesilverecho.zeropoint.api.render.animation;
...
public class Animate
{
	...
	public Animate update()
	{
		if (reversed)
		{
			if (time > min) time -= (MinecraftClient.getInstance().getTickDelta() * /*.001F **/ speed);
		} else if (time < max) time += (MinecraftClient.getInstance().getTickDelta() * /*.001F * */speed);
